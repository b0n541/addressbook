...
public class AddressbookModel extends IndexedContainer {
	...
	@Subscribe
	public void handleContactFilterEvent(ContactFilterEvent event) {
		removeAllContainerFilters();
		addContainerFilter(event.filter);
	}

	@Subscribe
	public void handleAddNewContactEvent(AddNewContactEvent event) {
		/*
	...
	private void initData() {
		...
		for (int i = 0; i < 1000; i++) {
			Object id = addItem();
			getContainerProperty(id, FNAME).setValue(
					fnames[(int) (fnames.length * Math.random())]);
			getContainerProperty(id, LNAME).setValue(
